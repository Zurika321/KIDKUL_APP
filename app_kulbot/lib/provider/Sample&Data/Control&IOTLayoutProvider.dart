...
class ControlLayoutProvider {
  ...
  static final Map<String, List<ControlItem>> _layoutsControl = {
  ...
  static final Map<String, List<ControlItem>> _layoutsIOT = {
    'IOT1': [
      ControlItem(
        id: 'Button_light',
        realId: 'Button_light1',
  ...
  static List<ControlItem> getLayout(String type, bool isControl) {
    final original = isControl ? _layoutsControl[type] : _layoutsIOT[type];
  ...
  static List<String> getAvailableTypes(bool isControl) {
    return isControl
        ? _layoutsControl.keys.toList()
        : _layoutsIOT.keys.toList();
  }

  /// Trả về map toàn bộ layout: { 'IOT1': [...], 'IOT2': [...] }
  static Map<String, List<ControlItem>> getAllLayouts(bool isControl) {
    return Map.from(isControl ? _layoutsControl : _layoutsIOT);
  }

  /// Kiểm tra một `type` có hợp lệ không
  static bool isValidType(String type, bool isControl) {
    return isControl
        ? _layoutsControl.containsKey(type)
        : _layoutsIOT.containsKey(type);
  }
}
