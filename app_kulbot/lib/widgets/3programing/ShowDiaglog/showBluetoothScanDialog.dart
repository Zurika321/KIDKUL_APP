...
class _BluetoothScanDialogState extends State<_BluetoothScanDialog> {
  ...
  Future<void> _requestBluetoothPermissions() async {
    try {
      ...
      if (!locationEnabled) {
        ...
        throw Exception("Location services (GPS) are off.");
      }
  ...
  Future<bool> checkBluetoothAndLocation(BuildContext context) async {
    ...
    showDialog(
      ...
      builder:
          (_) => AlertDialog(
            title: const Text("Need permission to continue"),
                ...
                children: [
                  if (!isBluetoothOn)
                    const ListTile(
                      ...
                      title: Text("Bluetooth is not on"),
                    ),
                  if (!isLocationOn)
                    const ListTile(
                      leading: Icon(Icons.location_off),
                      title: Text("Location Services (GPS) is not enabled"),
                    ),
                ],
              ),
            ),
                        ...
                        children: [
                          Expanded(
                            child: ListView.builder(
                              ...
                              itemBuilder:
                                  (_, i) =>
                                      _buildScanResultTile(_scanResults[i]),
                            ),
                          ),
                          if (_isScanning)
                            const Padding(
